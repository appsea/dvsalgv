            ...
            HttpService.getInstance().checkPlayStoreVersion().then((playStoreVersion: string) => {
                appVersion.getVersionCode().then((version: string) => {
                    if (Number(playStoreVersion) > Number(version)) {
                        dialogs.confirm({
                            title: "Notification",
                            message: "A latest version of LGV Theory Kit is now available on play store.",
                            okButtonText: "Upgrade",
                            cancelButtonText: "Remind me Later"
                        }).then((proceed) => {
                            if (proceed) {
                                utils.openUrl("https://play.google.com/store/apps/details?id=exuberant.dvsa.lttk");
                            }
                        });
                    }
                });
